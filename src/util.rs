...
use super::{fut::ModFuture, Module, RunCtx};

// Copy from tokio_util and adjust for ModFuture.

/// A reusable `Pin<Box<dyn ModFuture<Output = T> + Send + 'a>>`.
///
/// This type lets you replace the future stored in the box without
/// reallocating when the size and alignment permits this.
pub struct ReusableBoxFuture<T, M>
where
    T: Send + 'static,
    M: Module,
{
    boxed: NonNull<dyn ModFuture<M, Output = T> + Send>,
}

impl<T, M> ReusableBoxFuture<T, M>
where
    T: Send + 'static,
    M: Module,
{
    ...
    where
        F: ModFuture<M, Output = T> + Send,
    {
        let boxed: Box<dyn ModFuture<M, Output = T> + Send> = Box::new(future);
        ...
        F: ModFuture<M, Output = T> + Send,
    {
        ...
        let self_layout = {
            let dyn_future: &(dyn ModFuture<M, Output = T> + Send) = unsafe { self.boxed.as_ref() };
    ...
    pub fn get_pin(&mut self) -> Pin<&mut (dyn ModFuture<M, Output = T> + Send)> {
    ...
    pub fn poll(
        ...
        self.get_pin().poll(cx, module, runctx)
    }
}

impl<T, M> ModFuture<M> for ReusableBoxFuture<T, M>
where
    T: Send + 'static,
    M: Module,
{
    ...
    fn poll(
        ...
        Pin::into_inner(self).get_pin().poll(cx, module, runctx)
    }
}

// The future stored inside ReusableBoxFuture<'_, T> must be Send.
unsafe impl<T, M> Send for ReusableBoxFuture<T, M>
where
    T: Send + 'static,
    M: Module,
{
...
unsafe impl<T, M> Sync for ReusableBoxFuture<T, M>
where
    T: Send + 'static,
    M: Module,
{
...
impl<T, M> Unpin for ReusableBoxFuture<T, M>
where
    T: Send + 'static,
    M: Module,
{
...
impl<T, M> Drop for ReusableBoxFuture<T, M>
where
    T: Send + 'static,
    M: Module,
{
...
impl<T, M> fmt::Debug for ReusableBoxFuture<T, M>
where
    T: Send + 'static,
    M: Module,
{
